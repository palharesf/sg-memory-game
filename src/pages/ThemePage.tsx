...
function ThemeCard({ def, active, locked, onSelect }: ThemeCardProps) {
  return (
    <button
      ...
      onClick={onSelect}
      className={[
        "relative rounded-lg border-2 px-3 py-3 text-left transition-colors",
        locked
    ...
    >
      {active && !locked && (
        <span className="flex items-center gap-1 text-xs font-medium text-[var(--color-primary)] mb-1.5">
      ...
      {locked && (
        <span className="flex items-center gap-1 text-xs text-[var(--color-text-muted)] mb-1.5">
      ...
      {!active && !locked && <div className="mb-1.5 h-4" />}
      <p className="font-semibold text-sm text-[var(--color-text-bright)] mb-1 leading-snug">{def.name}</p>
      <p className="text-xs text-[var(--color-text-muted)] line-clamp-3 leading-snug">{def.cardDescription(def.pool)}</p>
    </button>
  );
}
              ...
              <div className="mb-3">
                ...
                {author?.url && (
                  <a href={author.url} target="_blank" rel="noopener noreferrer"
                    className="text-xs text-[var(--color-primary)] hover:underline break-all">
                    {author.url}
                  </a>
                )}
              </div>
